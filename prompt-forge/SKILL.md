...
用户只给了「85 分」这种模糊说法时，先把它翻译成上面三件事并确认——这一步用 `understand-first`。用户拒不给量化验收线时，取「异常处理」表中以 *用户拒不给量化验收线* 开头的那一行处理，不得自行编一条线顶上。

**迭代预算**同时钉死：默认最多 5 轮。预算是防止无限循环的硬闸，不是目标。

**🔴 CHECKPOINT · 🛑 STOP：进循环前逐条过——数字写出来了吗？口径第三方能复算吗？场景写明了吗？预算轮数钉死了吗？四项未经用户逐条确认，不进入循环。含糊的某一项，取「异常处理」表中以 *用户拒不给量化验收线* 开头的那一行处理。**
...
换全新案例集重跑，与上轮对照。凑不出符合 R4 的全新案例集时，取「异常处理」表中以 *案例集不够支撑下一轮* 开头的那一行处理——不得复用旧案例：
...
**本节是上述流程的例外条款：与正文规则冲突时——包括 R6「迭代预算是硬闸」、R4「每轮换新案例集」、R1 的跨领域适用性判定、`本轮结论不可信` 与「达标」的判定口径——以本表为准，其余未列出的规则同理。同一情形命中多行时，取更靠上的那一行。但每一处例外都必须在报告里显式标出，不得静默降级。**
