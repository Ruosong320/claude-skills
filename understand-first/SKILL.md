...
**🔴 CHECKPOINT — 样例交出去之前逐条过。任一条 ✗，先补齐再交，不得用「差不多」的样例换用户的确认。**
- **领域真的与用户举过的例子不同吗？** 同题换皮（女鞋 → 女鞋销量）不算跨领域。
- **每个样例都配了反例，且反例标了违反第几条吗？** 只给正例证明不了边界在哪。
- **至少一个样例踩在边界上吗？** 三个都显然成立的样例不算通过了这一步。
- **样例具体到能直接判对错吗？** 「一个好的拆分」这种不算样例。
