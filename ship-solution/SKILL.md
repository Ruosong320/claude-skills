...
**🔴 CHECKPOINT — 方案未确认、或验收标准不明确时，不得进入实现。** 逐条对照「前置条件」四问，缺哪条补哪条：缺方案 → 回到 real-solution-plan；缺用户确认或验收标准不明确 → 用 AskUserQuestion 与用户补齐；缺真实案例 → 按「方案执行不下去时的兜底」表中「方案给出的测试案例不可用」一行处理。补不齐不得进第 2 步。
...
- 典型示例：
  ...
  - 逻辑分支写漏（如忘记 `else` 处理）
  - 导入路径错误
  - 文件权限、网络沙箱等环境问题（仅限一次性能修好的；修不好就不是 B 类，按「方案执行不下去时的兜底」表中「验证跑不起来」一行处理，不得反复重试）
  - 格式化小瑕疵（多余空行、少一个换行符）
