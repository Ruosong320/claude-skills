...
**🔴 CHECKPOINT — 方案未确认、或验收标准不明确时，不得进入实现。** 逐条对照「前置条件」四问；任一条不成立，按该条自己的路由处理（缺方案 → 回到 real-solution-plan；缺真实案例 → 按「方案执行不下去时的兜底」表处理），不得直接进第 2 步。
...
- 项目环境变化（依赖版本、文件路径等）
- 用户在实现过程中明确要求调整（若新需求与已确认方案冲突，按「方案执行不下去时的兜底」表处理，不要直接照改）
...
代码必须满足：
- 符合项目现有代码风格（缩进、命名、导入顺序等）——与项目现有约定冲突时不要静默混用两套，按「方案执行不下去时的兜底」表处理
- 无冗余打印/日志（除非方案明确要求）
- 无未使用的函数、变量、导入
- 错误处理覆盖方案识别的边界情况
- 注释简洁，仅在复杂逻辑处添加
...
**案例拿不到、环境跑不起来、或标准全 PASS 但输出明显不对，不要在这一步自行降级——按「方案执行不下去时的兜底」表处理。**
...
- 典型示例：
  ...
  - 逻辑分支写漏（如忘记 `else` 处理）
  - 导入路径错误
  - 文件权限、网络沙箱等环境问题（仅限一次性能修好的；修不好就不是 B 类，按「方案执行不下去时的兜底」表第 2 行处理，不得反复重试）
  - 格式化小瑕疵（多余空行、少一个换行符）
...
3. **决策**
   ...
   **🛑 STOP — 决策树里「方案缺陷」与「需求误解」两支都必须先停：前者停止实现并报告方案缺陷，后者暂停实现并与用户确认标准。未确认前继续实现，就是被本 skill 明确禁止的补丁驱动开发。**
...
重构为通用实现后重新验证。硬编码嫌疑是系统性而非局部时（整个模块靠特例分支堆起来），停止逐个重构，按「方案执行不下去时的兜底」表处理。
...
**本节是上述流程的例外条款：与正文规则冲突时——包括核心原则 1「方案优先」、核心原则 2「必须真实验证」、核心原则 3「禁止补丁」、核心原则 4「禁止硬编码」、核心原则 5「小问题豁免」、2.2 允许偏离的清单、4.1 的一次修复预算、5.3「所有标准必须 PASS」——以本表为准，其余未列出的规则同理。但每一处例外都必须在报告中显式标出，不得静默降级。**
...
| 用户确认过的方案与用户中途提出的新需求冲突 | 指名冲突的是哪一条方案约束与哪一条新需求，让用户选：改方案还是压需求 | 用户不选且要求继续 → 停在第 2 步，不带冲突往下实现；已实现部分在报告中标出与哪条约束相抵 |
