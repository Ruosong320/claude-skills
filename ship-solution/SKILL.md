...
所有标准必须 PASS 才能报告为完成。按「方案执行不下去时的兜底」表处理过的每一处例外，在此逐条列出：哪条标准、什么例外、报告里在哪一段写明。
...
**本节是上述流程的例外条款：与正文规则冲突时——包括核心原则 1「方案优先」、核心原则 2「必须真实验证」、核心原则 4「禁止硬编码」、2.2 允许偏离的清单、4.1 的一次修复预算、5.3「所有标准必须 PASS」——以本表为准，其余未列出的规则同理。但每一处例外都必须在报告中显式标出，不得静默降级。**
