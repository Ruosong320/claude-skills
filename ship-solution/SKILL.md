...
**案例拿不到、环境跑不起来、或标准全 PASS 但输出明显不对，不要在这一步自行降级——按文末「方案执行不下去时的兜底」表处理。**
...
**本节是上述流程的例外条款：与正文规则（核心原则 1「方案优先」、核心原则 4「禁止硬编码」、2.2 允许偏离的清单、4.1 的一次修复预算）冲突时，以本表为准——但每一处例外都必须在报告中显式标出，不得静默降级。**
