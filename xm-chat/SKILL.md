---
name: xm-chat
description: Use this skill when the user wants to send Matrix chat messages (text/markdown/emote/notice/reply/edit), send files/images/stickers/locations, read history, listen for real-time messages, set up persistent chat daemon sessions, create AI virtual personas (ACP-based persistent chatbots via `xm chat persona` including concurrent dispatch, circuit breaker fallback, ask-in-room permission, turn timeout, daemon auto-reconnect, persona context env vars for agents, diagnostic commands like check/ping/trace/watch/explain), manage E2E encryption keys, configure Matrix appservice virtual identities, manage chat profiles/presence, list room members, search users, or set up event-triggered commands with context injection, batching, and agent session continuity (claude/codex). Covers all `xm chat` subcommands and patterns including bot development workflows. Also covers event filter configuration: prefix matching, room/sender exclusion, multi-value OR rooms/froms, OR substring/regex matching, sender domain filtering, body length constraints, and trigger-name @mention detection. Also trigger when the user mentions chat, messaging, Matrix, IM, bot, daemon session, chat trigger, virtual user, virtual persona, AI persona, chatbot, DM, room members, trigger pipeline, agent mode, XM_AGENT_SESSION, Monitor mode, Claude Monitor, XM_CHAT_USERNAME, XM_CHAT_PASSWORD, XM_CHAT_TOKEN, multi-device Matrix login, file send, sticker, location, emote, edit message, event filter, message filter, prefix filter, domain filter, persona debugging, persona diagnostic, router check, event tracing, or any room-based communication task.
---
