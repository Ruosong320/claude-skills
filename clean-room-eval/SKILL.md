...
被测物版本不可判时，取「评测跑不下去时的兜底」表中以 *被测物版本不可判* 开头的那一行；判定口径第三方复算不了时，取以 *案例跑完才发现判定口径第三方复算不了* 开头的那一行——两者都会让结论不可复算，不得带病开跑。
...
- **判定口径还写不成第三方能复算的一句话吗？** → 取兜底表中以 *案例跑完才发现判定口径第三方复算不了* 开头的那一行。
...
**🛑 STOP — 每个子 agent 的 prompt 发出前，对照上面这份禁止清单过一遍。任何一条混进去了，这一例的产出即作废重跑——被污染的样本不是「效果差」，是无效数据。**
