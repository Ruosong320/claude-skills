...
被测物版本不可判、或判定口径第三方复算不了时，按「评测跑不下去时的兜底」表处理——两者都会让结论不可复算，不得带病开跑。
...
**🔴 CHECKPOINT — 案例清单落盘之后、开第一个子 agent 之前，过一遍。任一命中，先走「评测跑不下去时的兜底」表；开跑之后再补这三件事，前面的案例就白跑了。**

- **案例清单还没落盘吗？**（跑完再挑案例等于作弊）→ 先落盘再开跑。
- **判定口径还写不成第三方能复算的一句话吗？** → *口径不可复算行*。
- **被测物的版本或哈希还没记下来吗？** → *版本不可判行*。
...
并发跑。子 agent 之间不得互相看见。开不起来或中途失败，按「评测跑不下去时的兜底」表中「子 agent 跑不起来」一行处理——那是样本损耗，不是案例不通过。

### 4. 并行建人类基准

主 agent 在子 agent 跑的同时联网检索：**这些案例，业内/人类通常怎么做**。找真实的行业分类法、标准、成熟方法论、公开实现，而不是自己凭感觉想一套标准。

没有基准的评测只能说出「我觉得不太好」；有基准才能说出「行业标准分 7 类，它只覆盖了 3 类，漏的都是供应链侧」。检索不到基准时，按「评测跑不下去时的兜底」表处理，不得凭感觉编一套标准顶上去。
...
把问题标签按出现次数排序。没有一个标签达到共现阈值时，按「评测跑不下去时的兜底」表中「没有共性问题」一行处理。
...
| 被测物版本不可判（同名文件多份、无版本标记、评测中途被改） | 取最新一份并记哈希写进报告；能问到用户就直接问 | 仍不可判 → 停止评测：版本不可关的评测结论不可复算，先请用户指定唯一被测版本再开跑 |
| 案例跑完才发现判定口径第三方复算不了 | 把口径重写成可复算的一句话，然后**重跑**——不得在跑完后悄悄改口径去适配已有产出 | 口径始终无法复算 → 降级为「探索性结论」，报告首段显式声明口径不可复算、结论不可作为改动依据 |
