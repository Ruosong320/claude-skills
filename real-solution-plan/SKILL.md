...
没有先验分析结论而问题又复杂 → 按「方案写不下去时的兜底」表中「没有先验分析结论」一行处理。
...
候选方案之间对比不出实质差异 → 按「方案写不下去时的兜底」表中「对比不出实质差异」一行处理；用户已有明确选型与检索结果冲突 → 按同表中「用户已有明确技术选型」一行处理。不得硬凑一个推荐。
...
**🔴 CHECKPOINT — 输出方案前逐条过上面这张自检表。任一条 ✗，先走「方案写不下去时的兜底」表，不得带着 ✗ 输出。**
...
| 没有先验分析结论（无 analyze-only 产出）而问题又复杂 | 先调用 analyze-only 产出结论，再回到第 2 步 | 用户要求跳过 → 照做，但方案首段写明「无先验分析」，「依据来源」的先验分析栏标「无」 |
| 项目内找不到同类功能的现有实现（新项目、空仓库、无关模块） | 在「依据来源」写「无项目内先例」，选型主依据改为检索到的真实案例 | 连检索也没有 → 让用户提供参考项目或一段可对照的代码，不得凭印象编项目现状 |
...
- **前置：analyze-only** - 如果问题复杂，先调用 analyze-only 产出分析结论，再基于结论产出方案
